...
@compiled
def _min_sample_periods(min_sample, min_periods, default_min_periods = None):
    if min_periods is not None:
        min_sample = 0 if min_sample is None else min_sample
    else:
        min_periods = default_min_periods
        min_sample = 0.25 if min_sample is None else min_sample
    return min_sample, min_periods
    ...
    for i in range(a.shape[0]):
        if np.isnan(a[i]):
            ...
            if time[i] == t:
                ...
                t = time[i]
            i0 = i
            res[i] = np.nan if t0 == 0 or n1<min_periods else t1/t0
    return res, t, t0, t1, n1
    ...
    if apply_max:
        ...
        for i in range(a.shape[0]):
            if np.isnan(a[i]):
                ...
                res[i] = np.nan if t0 == 0 or n1<min_periods else np.sqrt(t2/t0)
        ...
        for i in range(a.shape[0]):
            if np.isnan(a[i]):
                ...
                i0 = i; ai0 = ai
                res[i] = np.nan if t0 == 0 or n1<min_periods else np.sqrt(t2/t0)        
    return res, t, t0, t2, n1



@pd2np
@compiled
def _ewmstd(a, n, time, wgt, t = np.nan, t0 = 0, t1 = 0, t2 = 0, w2 = 0, min_sample = None, bias = False, exc_zero = False, max_move = None, calculator = stdev_calculation_ewm, min_periods = None, n1 = 0, n0 = 0):
    """
    t = np.nan; t0 = 0; t1 = 0; t2 = 0; w2 = 0; min_sample = 0.25; bias = False; 
    exc_zero = False; max_move = None; calculator = stdev_calculation_ewm; 
    min_periods = 0; n1 = 0
    """
    min_sample, min_periods = _min_sample_periods(min_sample, min_periods, 3)
    w = _w(n)
    v = (1-w)*wgt
    res = np.empty_like(a)
    apply_max = max_move is not None
    ...
    if apply_max:
        ...
        for i in range(a.shape[0]):
            if np.isnan(a[i]):
                ...
                vol = np.nan if n0 < min_sample or n1<min_periods else calculator(t0, t1_, t2_, w2 = w2, bias = bias)
    else:
        for i in range(a.shape[0]):
            if np.isnan(a[i]):
                res[i] = np.nan
...
def _ewmx(a, b, n, wgt, time, dtype, NAN = np.nan, ONE = 1.0, t = None,
          a1 = None, a2 = None, b1 = None, b2 = None, ab = None, prev_a = None, prev_b = None, 
             w1 = None, w2 = None, n0 = None, min_sample = None, bias = False, overlapping = 1, 
             calculation = cor_calculation_ewm, ffill = True, n1 = None, min_periods = None):
    """
    ...
    """
    min_sample, min_periods = _min_sample_periods(min_sample, min_periods, 0)
    x = a.shape[1]
    y = b.shape[1]
                ...
                if ~np.isnan(a[i,j]) and ~np.isnan(b[i,k]):
                    if ~np.isnan(prev_a[j,k,-1]) and ~np.isnan(prev_b[j,k,-1]):
                        if ~np.isnan(vi) and ~np.isnan(time[i]) and time[i] == t[j,k]:                            
                            ...
                            t[j,k] = time[i]
                    if n0[j,k] >= min_sample and n1[j,k] >= min_periods:
                        c = calculation(t0 = w1[j,k], a1 = a1[j,k], a2 = a2[j,k], 
                                                           w2 = w2[j,k], 
                                                           b1 = b1[j,k], b2 = b2[j,k], ab = ab[j,k], bias = bias) 
                    ...
                    prev_b[j,k,0] = b[i,k]                 
                if np.isnan(c):
                    c = res0[i-1,j,k] if ffill and i>0 else NAN
                else:
                    c = ONE * c
                res0[i,j,k] = c
...
def _ewmx2(a, b, n, wgt, time, dtype, NAN = np.nan, ONE = 1.0, t = None,
          a1 = None, a2 = None, b1 = None, b2 = None, ab = None, prev_a = None, prev_b = None, 
             w1 = None, w2 = None, n0 = None, min_sample = None, bias = False, overlapping = 1, 
             calculation = LR_calculation_ewm, ffill = True, n1 = None, min_periods = None):
    """
    cross-sectional correlation of a and b
    ...
    for j in range(x):
        for k in range(y):
            for i in range(a.shape[0]):
                ...
                if np.isnan(c1):
                    c1 = res0[i-1, j, k] if ffill and i>0 else NAN
                else:
                    c1 = ONE * c1
                if np.isnan(c2):
                    c2 = res1[i-1, j, k] if ffill and i>0 else NAN
                else:
                    c2 = ONE * c2
...
@compiled
def _ewmcorrelation(a, n, wgt, dtype, NAN = np.nan, ONE = 1.0, a0 = None, a1 = None, a2 = None, aa = None, prev = None, w2 = None, n0 = None, min_sample = None, bias = False, overlapping = 1, ffill = True, min_periods = None, n1 = None):
    """
    ...
    n1 = np.zeros((m,m)) if n1 is None else n1
    prev = np.full((m,m,overlapping), np.nan) if prev is None else prev
    for i in range(a.shape[0]):
        for j in range(m):
            if np.isnan(a[i,j]):
                if ffill and i>0:
                    res[i,j,j] = res[i-1,j,j]
                    for k in range(j):
                        res[i,j,k] = res[i,k,j] = res[i-1,j,k]                    
            else:
                a0[j,j] = a0[j,j] * p + v[i]
                n0[j,j] = n0[j,j] * p + (1-w)
                n1[j,j] += 1
                if n0[j,j] > min_sample and n1[j,j]>min_periods:
                    res[i,j,j] = ONE
                for k in range(j):
                    c = np.nan
                    if ~np.isnan(a[i,k]):
                        if ~np.isnan(prev[j,k,-1]) and ~np.isnan(prev[k,j,-1]):
                            ...
                            if n0[j,k] >= min_sample and n1[j,k]>=min_periods:
                                c = cor_calculation_ewm(t0 = a0[j,k], 
                                                                                  a1 = a1[j,k], 
                                                                                  a2 = a2[j,k], 
                                                                                  w2 = w2[j,k], 
                                                                                  b1 = a1[k,j], 
                    ...
                    if np.isnan(c):
                        c = res[i-1,j,k] if ffill and i>0 else NAN
                    else:
                        c = ONE * c
...
@compiled
def _ewmcovariance(a, n, wgt, dtype, NAN = np.nan, ONE = 1.0, a0 = None, a1 = None, aa = None, prev = None, n0 = None, min_sample = None, bias = False, overlapping = 1, ffill = True, n1 = None, min_periods = None):
    """
            ...
            if np.isnan(a[i,j]):
                ...
                for k in range(j+1):
                    ...
                    if np.isnan(c):
                        ...
                        c = ONE * c
                    res[i,j,k] = res[i,k,j] = c
    return res, a0, a1, aa, prev, n0, n1



def ewmcovariance_(a, n, min_sample = None, bias = False, overlapping = 1, instate = None, join = 'outer', method = None, wgt = None, dtype = None, ffill = True, min_periods = None):
    """
    ...
    Example
    min_sample = 0.25; bias = False; overlapping = 1; instate = None; join = 'outer'; method = None; wgt = None; dtype = None; ffill = True
    ...
    if isinstance(arr, np.ndarray):
        res, a0, a1, aa, prev, n0, n1 = _ewmcovariance(arr, n, wgt = wgt, min_sample = min_sample, bias = bias, overlapping = overlapping, dtype = dtype or np.float32, ffill = ffill, min_periods = min_periods, **state)
        ...
        res, a0, a1, aa, prev, n0, n1 = _ewmcovariance(arr.values, n, wgt = wgt, min_sample = min_sample, overlapping = overlapping, bias = bias, dtype = dtype or np.float32, ffill = ffill, min_periods = min_periods, **state)
        state = dictattr(a0=a0, a1=a1, aa=aa, prev = prev, n0 = n0)
...
def ewmcovariance(a, n, min_sample = None, bias = False, overlapping = 1, instate = None, join = 'outer', method = None, wgt = None, dtype = None, ffill = True, min_periods = None):
    """
    ...
    """
    return ewmcovariance_(a, n, wgt = wgt, min_sample = min_sample, bias = bias, overlapping = overlapping, instate = instate , join = join, method = method, dtype = dtype, ffill = ffill, min_periods = min_periods).get('data')



def ewmcovar_(a, n, min_sample = None, bias = False, overlapping = 1, instate = None, join = 'outer', method = None, wgt = None, dtype = None, ffill = True, min_periods=None):
    """
    ...
    return ewmcovariance_(cumsum(arr), wgt = wgt, n = n, min_sample=min_sample, overlapping = overlapping, bias = bias, instate = state, join = join, method = method, dtype = dtype, ffill = ffill, min_periods = min_periods)   

ewmcovar_.output = ['data', 'columns', 'index', 'state']


def ewmcovar(a, n, min_sample = None, bias = False, overlapping = 1, instate = None, join = 'outer', method = None, wgt = None, dtype = None, ffill = True, min_periods = None):
    """
    This calculates a full covariance matrix as a timeseries. 
...
@pd2np
@compiled
def _ewmskew(a, n, time, wgt, bias = 0, t = np.nan, t0 = 0, t1 = 0, t2 = 0, t3 = 0, min_sample = 0.25, min_periods = None, n1 = 0, n0 = None):
    min_sample, min_periods = _min_sample_periods(min_sample, min_periods, 4)
    w = _w(n)
    v = (1-w) * wgt
...
@loop_all
def _ewmat(a, n, wgt = None, time = None, t = None, t0 = 0, t1 = 0, min_periods = 0, n1 = 0):
    time = clock(a, time, t)
    wgt = _wgt(a, wgt)
    t = 0 if t is None or np.isnan(t) else t
    return _ewma(a = a, n = n, wgt = wgt, time = time, t = t, t0 = t0, t1 = t1, min_periods = min_periods, n1 = n1)

@loop_all
def _ewmrmst(a, n, wgt = None, time = None, t = None, t0 = 0, t2 = 0, exc_zero = False, max_move = None, min_periods = 0, n1 = 0):
    time = clock(a, time, t)
    wgt = _wgt(a, wgt)
    t = 0 if t is None or np.isnan(t) else t
    if is_num(max_move):
...
@loop_all
def _ewmstdt(a, n, wgt = None, time = None, t = None, t0 = 0, t1 = 0, t2 = 0, w2 = 0, min_sample = None, bias = False, exc_zero = False, max_move = None, calculator = stdev_calculation_ewm, min_periods = None, n1 = 0, n0 = 0):
    """
...
def _ewmxt(a, b, n, wgt = None, time = None, t = None, a1 = None, a2 = None, b1 = None, b2 = None, 
              ...
              min_sample = None, bias = False, overlapping = 1, 
              calculation = cor_calculation_ewm, dim = None, dtype = None, ffill = True, min_periods=None, n1 = None):
    """
    ...
    if dim == 1:
        res, a1, a2, b1, b2, ab, prev_a, prev_b, w1, w2, n0, t, n1 = _ewmx(a = a, b = b, n = n, wgt = wgt, time = time,
                       ...
                       min_periods = min_periods, n1 = n1,
                       ab = ab, min_sample=min_sample, bias = bias, overlapping = overlapping, calculation=calculation, dtype = dtype, ffill = ffill, NAN = NAN, ONE = ONE)
        res = res[0]
    else:
        res, a1, a2, b1, b2, ab, prev_a, prev_b, w1, w2, n0, t, n1 = _ewmx2(a = a, b = b, n = n, wgt = wgt, time = time,
                       a1 = a1, a2 = a2, b1 = b1, b2 = b2, w1 = w1, w2 = w2, n0 = n0, prev_a = prev_a, prev_b = prev_b,
                       min_periods = min_periods, n1 = n1,
                       ab = ab, min_sample=min_sample, bias = bias, overlapping = overlapping, calculation=calculation, dtype = dtype, ffill = ffill, NAN = NAN, ONE = ONE)
    res = _reshape(res, reshape_a = reshape_a, reshape_b = reshape_b)
    return res, a1, a2, b1, b2, ab, prev_a, prev_b, w1, w2, n0, t, n1
...
@loop_all
def _ewmskewt(a, n, wgt = None, time = None, t = None, bias = False, t0 = 0, t1 = 0, t2 = 0, t3 = 0, min_sample = None, min_periods = None, n1 = 0, n0 = 0):
    time = clock(a, time, t)
    wgt = _wgt(a, wgt)
    t = 0 if t is None or np.isnan(t) else t
...
def ewma_(a, n, time = None, data = None, instate = None, wgt = None, min_periods = 0):
    """
    ...
    """
    state = instate or {}
    return _data_state(['data', 't', 't0', 't1', 'n1'], _ewmat(a = a, n = n, time = time, wgt = wgt, min_periods = min_periods, **state))

ewma_.output = ['data', 'state']


def ewma(a, n, time = None, axis = 0, data = None, state = None, wgt = None, min_periods = 0):
    """
    ...
    """
    state = state or {}
    return first_(_ewmat(a=a, n=n, wgt = wgt, time = time, axis=axis, min_periods = min_periods, **state))


def ewmrms_(a, n, time = None, axis = 0, data = None, exc_zero = False, max_move = None, instate = None, wgt = None, min_periods = 0):
    """
    ...
    """
    state = instate or {}
    return _data_state(['data', 't', 't0', 't2', 'n1'],
                       _ewmrmst(a, n, time = time, axis=axis, exc_zero = exc_zero, max_move = max_move, wgt = wgt, min_periods = min_periods, **state))

ewmrms_.output = ['data', 'state']

def ewmrms(a, n, time=None, axis=0, data = None, exc_zero = False, max_move = None, state = None, wgt = None, min_periods = 0):
    """
    ...
    """
    state = state or {}
    return first_(_ewmrmst(a, n = n, wgt = wgt, time = time, axis=axis, exc_zero = exc_zero, max_move = max_move, min_periods=min_periods, **state))

def ewmstd_(a, n, time = None, min_sample=None, bias = False, axis=0, data = None, exc_zero = False, max_move = None, instate = None, wgt = None, min_periods=None):
    """
    ...
    return _data_state(['data', 't', 't0', 't1', 't2', 'w2', 'n1', 'n0'],
                       _ewmstdt(a, n = n, wgt = wgt, 
                                ...
                                exc_zero = exc_zero, 
                                max_move = max_move, 
                                calculator = stdev_calculation_ewm, 
                                min_periods = min_periods, **state))

ewmstd_.output = ['data', 'state']

def ewmstd(a, n, time = None, min_sample=None, bias = False, axis=0, data = None, exc_zero = False, max_move = None, state = None, wgt = None, min_periods = None):
    """
    ...
    """
    state = state or {}
    return first_(_ewmstdt(a, n = n, wgt = wgt, time = time, min_sample=min_sample, bias = bias, axis=axis, exc_zero = exc_zero, max_move = max_move,  calculator = stdev_calculation_ewm, min_periods = min_periods, **state))


def ewmvar_(a, n, time = None, min_sample=None, bias = True, axis=0, exc_zero = False, data = None, instate = None, max_move = None, wgt = None, min_periods=None):
    """
...
def ewmvar(a, n, time = None, min_sample=None, bias = True, axis=0, exc_zero = False, data = None, state = None, max_move = None, wgt = None, min_periods = None):
    """
    ...
    """
    state = state or {}
    return first_(_ewmstdt(a, n = n, wgt = wgt, time = time, min_sample=min_sample, bias = bias, axis=axis, exc_zero = exc_zero, max_move = max_move, calculator = variance_calculation_ewm, min_periods = min_periods, **state))
...
def ewmx_(a, b, n, time = None, min_sample = None, bias = True, data = None, instate = None, wgt = None, 
             overlapping = 1, join = 'outer', method = None, calculation = cor_calculation_ewm, is_returns = False, dim = None, dtype = None, ffill = True, min_periods = None):
    """
...
def ewmxcor_(a, b, n, time = None, min_sample = None, bias = True, data = None, instate = None, wgt = None, overlapping = 1, 
             join = 'outer', method = None, is_returns = False, dtype = None, ffill = True, min_periods = None):
...
def ewmxcor(a, b, n, min_sample = None, bias = True, data = None, state = None, 
            wgt = None, overlapping = 1, is_returns = False, dtype = None, ffill = True, min_periods = None):
    """
    ...
    :Example: ffill for integers
    -------------------------
    >>> ts = ewmxcor(a, b, n = 10, dtype = np.int8, ffill = False)
    >>> assert ts.iloc[-1] == 127
    >>> ts = ewmxcor(a, b, n = 10, dtype = np.int8, ffill = True)
    >>> assert ts.iloc[-1] != 127 and ts.iloc[-1] == ts.iloc[-2]

    >>> x = pd.Series(np.random.normal(0,1,9000), drange(-8999))
    >>> y = pd.Series(np.random.normal(0,1,9000), drange(-8999))
    >>> y.iloc[-10:] = np.nan
    >>> a = df_concat([x,y], ['x', 'y']).values
    >>> n = 10
    >>> res = ewmcorr(a, n, dtype = np.int8)
    >>> assert res[-1][0,1]!=127 and res[-1][0,1] == res[-2][0,1]

    >>> res = ewmcovariance(a, n)
...
def ewmxLR_(a, b, n, time = None, min_sample = None, bias = True, axis = 0, c = None, m = None, instate = None, wgt = None, is_returns = False, dtype = None, ffill = True, min_periods = None):
    """
...
def ewmxLR(a, b, n, time = None, min_sample = None, bias = True, state = None, wgt = None, is_returns = False, dtype = None, ffill = True, min_periods = None):
    """
...
def ewmskew_(a, n, time = None, bias = False, min_sample = None, axis=0, data = None, instate = None, wgt = None, min_periods = None):
    """
...
def ewmskew(a, n, time = None, bias = False, min_sample = None, axis=0, data = None, state = None, wgt = None, min_periods = None):
    """
